        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (!isPrems && plugin.diamond && global.db.data.users[m.sender].diamond < plugin.diamond * 1) {
                     this.reply(m.chat, `JBOT money finished \n use the following JBOT command to buy money \n*${usedPrefix}todiamond* <amount`, m)
                    continue // Limit habis
                }
                if (plugin.level > _user.level) {
                    this.reply(m.chat, `JBOT required level ${plugin.level} to use this JBOT command. \nyour level ${_user.level}`, m)
                    ...
                    if (e) {
                        ...
                        if (e.name)
                            for (let [jid] of global.owner.filter(([number, _, isDeveloper]) => isDeveloper && number)) {
                                let data = (await conn.onWhatsApp(jid))[0] || {}
                                if (data.exists)
                                    m.reply(`*JBOT Plugin:* ${m.plugin}\n*JBOT Sender:* ${m.sender}\n*JBOT Chat:* ${m.chat}\n*JBOT Command:* ${usedPrefix}${command} ${args.join(' ')}\nJBOT *Error Logs:*\n\n\`\`\`${text}\`\`\``.trim(), data.jid)
                            }
                        m.reply(text)
                    }
                } finally {
...
export async function participantsUpdate({ id, participants, action }) {
   ...
   case 'add':
        ...
        case 'promote':
        case 'promover':
            text = (chat.sPromote || this.spromote || conn.spromote || '@user is now administrador by JBOT')
        case 'demote':
        case 'degradar':
            if (!text)
                text = (chat.sDemote || this.sdemote || conn.sdemote || '@user not now an administrador JBOT')
...
export async function groupsUpdate(groupsUpdate) {
    ...
    for (const groupUpdate of groupsUpdate) {
        ...
        if (groupUpdate.subject) text = (chats.sSubject || this.sSubject || conn.sSubject || '```Subject has been changed ```\n@subject').replace('@subject', groupUpdate.subject)
        if (groupUpdate.icon) text = (chats.sIcon || this.sIcon || conn.sIcon || '```Icon has been changed ```').replace('@icon', groupUpdate.icon)
        if (groupUpdate.revoke) text = (chats.sRevoke || this.sRevoke || conn.sRevoke || '```Group link has been changed```\n@revoke').replace('@revoke', groupUpdate.revoke)
...
global.dfail = (type, m, conn) => {
    let msg = {
        rowner: '*ᴏɴʟʏ JBOT ᴅᴇᴠᴇʟᴏᴘᴇʀ* • 🇱🇰 This command can only be used by the *Creator of JOSH*',
        owner: '*ᴏɴʟʏ JBOT ᴏᴡɴᴇʀ* • 🇱🇰 This command can only be used by the *Bot Owner JOSH',
        mods: '*ᴏɴʟʏ JBOT ᴍᴏᴅᴇʀᴀᴛᴏʀ* •🇱🇰 This function is only for *For JBOT Bot moderators*',
        premium: '*ᴏɴʟʏ JBOT ᴘʀᴇᴍɪᴜᴍ User* • 🇱🇰 This command is for *JBOT Premium members only',
        group: '*JBOT ɢʀᴏᴜᴘ ᴄʜᴀᴛ* • 🇱🇰 This command can only be used in JBOT groups',
        private: '*JBOT ᴘʀɪᴠᴀᴛᴇ ᴄʜᴀᴛ* • 🇱🇰 This command can only be used in the *private chat of the jBot*',
        admin: '*ᴏɴʟʏ JBOT ᴀᴅᴍɪɴ* • 🇱🇰 This command is only for *JBOT Group Admins*',
        botAdmin: '*ᴏɴʟʏ JBOT ʙᴏᴛ ᴀᴅᴍɪɴ* • 🇱🇰 To use this command I must be *JBOT Admin!*',
        unreg: '*ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ʀᴇɢɪsᴛᴇʀᴇᴅ with JBOT* • 🇱🇰 Sign in to JBOT,use this feature Type කරපන්:\n\n*/reg name.age*\n\n🇱🇰 Example : *.reg DINUWA.17*', 
        restrict: '*ʀᴇsᴛʀɪᴄᴛby J WA BOT* • This feature is *disabled*by J WA BOT',
    }[type]
